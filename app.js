...
function showGoalOnHover(e) {
  for (var i = 0; i < circles.length; i++) {
    if (e.target == circles[i]) {
      todoItem.innerText = circles[i].info.goal;
    }
  }
...
function appearAnimations() {
  ...
  setTimeout(function () {
    showTodoItem.style.display = "none";
    todoInfo.style.display = "flex";
    todoInfo.classList.add("scaleUp");
  }, 150);
}
...
function underlineBlack() {
  document.querySelector(".goal-underline").style.backgroundColor =
    "var(--secondary-color)";
}
